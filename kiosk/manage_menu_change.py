...
def view_menu():
    ...
    if request.method == 'GET':
        if request.args.get('burger'):
            ...
            info = c.fetchall()
            db.close()
            return render_template('/manage_menu/change_menu.html', data=info, menu_data=0)
...
def view_detail():
    ...
    if request.method=='POST':
        ...
        db.close()
        return render_template('/manage_menu/change_menu.html', data=info, menu_data=menu_data)
...
def change_menu():
    ...
    if request.method=='POST':
        ...
        db.commit()
        db.close()
        return redirect(url_for('change_menu.view_menu'))
